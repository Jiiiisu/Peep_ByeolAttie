import React, {useRef, createRef, useMemo, useEffect} from 'react';
import {
  Text,
  View,
  StyleSheet,
...
import {
  ...
  fillEmpty,
  asPickerFormat,
} from '../constants/TimePicker_utils';
import {
  MERIDIEM_ITEMS,
  MINUTE_ITEMS,
  HOUR_ITEMS,
...
const TimePicker = ({
  ...
  initialDate.setMinutes(initialMinute ? parseInt(initialMinute) : 0);
  ...
  const refs = useRef(Array.from({length: 3}).map(() => createRef()));
  ...
  useEffect(() => {
    const meridiem = isPM(initialDate) ? '오후' : '오전';
    const hour = String(
      isPM(initialDate) ? initialDate.getHours() - 12 : initialDate.getHours(),
    ).padStart(2, '0');
    const minute = String(initialDate.getMinutes()).padStart(2, '0');
